# NOUMEN --- Definitive Build Plan

> Version: 1.0 | Based on Architecture v3.2.3 | 29 Active Axioms | 3 Agents
> This document is the single source of truth for implementing NOUMEN on Solana.
...
- **Why separate**: The Auditor must be independent of the agents it evaluates. It has no CPI calls to any other NOUMEN program (except reading PDAs). No agent can influence its own truth labels or accuracy scores.
...
| T2 | Unauthorized CPI | CRITICAL | Malicious program invokes NOUMEN instructions pretending to be an authorized caller | All instructions require explicit signer checks. CPI targets are verified against program IDs stored in AeonConfig (treasury_program, proof_program). Anchor's `Program<>` type validates program ownership. | A0-7 |
...
| T12 | Agent impersonation | HIGH | Malicious entity creates a fake agent that mimics a legitimate NOUMEN agent | All agents registered in AgentManifest with unique agent_id and authority keypair. PDA seeds include agent_id, preventing collision. External agents are read-only on NOUMEN PDAs (A0-7). Agent creation restricted to AEON (A0-1). | A0-1, A0-7 |
...
| A0-7 | External programs cannot write to NOUMEN PDAs (Anchor owner check). Read access is public on Solana. | On-chain (structural) | N/A |
...
- **Integration pattern**: Off-chain Risk Engine reads Pyth price accounts directly from Solana. No on-chain oracle consumption in NOUMEN programs (keeps programs simple, avoids oracle dependency for core operations).
...
Dark mode primary. Premium fintech aesthetic targeting the quality bar set by Drift Protocol and Jupiter Exchange. The UI must communicate trust, precision, and transparency -- reflecting the axiom-driven governance that underlies NOUMEN.
...
- **Purpose**: Marketing page. Explains NOUMEN, the axiom system, agent architecture, and CCS model.
- **Layout**: Full-width hero with gradient mesh background. Scroll-driven sections: Hero -> What is NOUMEN -> The 3 Agents -> 29 Axioms Grid -> CCS Transparency -> CTA.
...
NOUMEN operates entirely in native SOL. There is no governance token, no utility token, and no staking mechanism. All economic flows are SOL-denominated and settled on-chain through the treasury program.
...
```mermaid
gantt
    title NOUMEN Implementation Roadmap
